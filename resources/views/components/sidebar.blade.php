        ...
        <nav class="space-y-3">
            ...
            @foreach ($plugins as $plugin)
                @can('read ' . $plugin->get('alias'))
                    <Link href="{{ route('dashboard.' . $plugin->get('alias') . '.index') }}"
            ...
            @can('read reports')
                <x-splade-dropdown class="w-full">
                    <x-slot:trigger>
                        <a class="text-sm py-3 my-0 mx-2 flex items-center whitespace-nowrap px-4 font-medium text-gray-500 dark:text-white hover:bg-indigo-50 dark:hover:bg-indigo-400 rounded-lg shadow-none transition-colors ease-in-out"
                            href="#">
            ...
            @if (Auth::user()->getRole() == 'super-admin')
                <a href="{{ route('admin.coin.requests') }}"
                    ...
                    <i class="fa-solid fa-coins"></i>
                    <span class="mx-2 text-sm">Request coins list</span>
                </a>
            @endif
        </nav>
    </div>
